...
@Component({
  selector: 'app-ticketsfilter',
  templateUrl: './ticketsfilter.component.html',
  styleUrls: ['./ticketsfilter.component.css']
})
export class TicketsfilterComponent implements OnInit {
  ...
  setResult(res) {
    ...
    for (let i = 0; i < res[0].tickets.length; i++) {
      if (this.month === res[0].tickets[i].created_at.substring(0, 7)) {
        for (let x = 0; x < res[1].organizations.length; x++) {
          if (res[1].organizations[x].id === res[0].tickets[i].organization_id) {
            this.tickets.push({ Creado: res[0].tickets[i].created_at, ticket_number: res[0].tickets[i].id, status: res[0].tickets[i].status, organization: res[1].organizations[x].name, entry: res[0].tickets[i].via.channel })
          }
        }
      }
    }
  ...
  filterdate(inicio:Date, fin: Date){
